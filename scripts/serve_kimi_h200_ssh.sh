...
LOG="$LOG_DIR/vllm-kimi-h200.log"
...
ssh -o BatchMode=yes "$HEAD" "cd '$REPO_DIR'; mkdir -p '$LOG_DIR'; ENV_LIB=lib_env.sh RAY=1 MODEL='$MODEL' API_PORT='$API_PORT' MAX_MODEL_LEN='${MAX_MODEL_LEN:-32768}' GPU_UTIL='${GPU_UTIL:-0.90}' TP='${TP:-4}' PP='${PP:-2}' ENFORCE_EAGER='${ENFORCE_EAGER:-1}' EXTRA_ARGS='${EXTRA_ARGS:---skip-mm-profiling --mm-processor-cache-gb 0}' nohup bash '$S/vllm_kimi.sh' '$HEAD_IP' '$RAY_PORT' > '$LOG' 2>&1 < /dev/null & echo launched pid \$!"
