...
LOG="$LOG_DIR/sglang-glm.log"
WORKER_LOG="$LOG_DIR/sglang-glm-worker.log"
...
ssh -o BatchMode=yes "$WORKER" "cd '$REPO_DIR'; mkdir -p '$LOG_DIR'; $ENVS nohup bash '$S/sglang_glm.sh' 1 '$HEAD_IP' > '$WORKER_LOG' 2>&1 < /dev/null & echo worker pid \$!"
echo "launching SGLang head (rank 0) on $HEAD"
ssh -o BatchMode=yes "$HEAD" "cd '$REPO_DIR'; mkdir -p '$LOG_DIR'; $ENVS nohup bash '$S/sglang_glm.sh' 0 '$HEAD_IP' > '$LOG' 2>&1 < /dev/null & echo head pid \$!"
