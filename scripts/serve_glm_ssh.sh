...
echo "launching vllm on $HEAD (log: logs/vllm-glm.log)"
ssh -o BatchMode=yes "$HEAD" "cd '$REPO_DIR'; MODEL='$MODEL' API_PORT='$API_PORT' MAX_MODEL_LEN='${MAX_MODEL_LEN:-131072}' GPU_UTIL='${GPU_UTIL:-0.90}' ENFORCE_EAGER='${ENFORCE_EAGER:-}' NO_MTP='${NO_MTP:-}' PERF='${PERF:-}' nohup bash '$S/vllm_glm.sh' '$HEAD_IP' '$RAY_PORT' > '$REPO_DIR/logs/vllm-glm.log' 2>&1 < /dev/null & echo launched pid \$!"
echo "watch:    ssh $HEAD tail -f $REPO_DIR/logs/vllm-glm.log"
