...
ssh -o BatchMode=yes "$NODE" "cd '$REPO_DIR'; mkdir -p '$LOG_DIR'; VENV_DIR='${VENV_DIR:-}' CUDA13_DIR='${CUDA13_DIR:-}' MODEL='$MODEL' API_PORT='$API_PORT' MAX_MODEL_LEN='${MAX_MODEL_LEN:-131072}' GPU_UTIL='${GPU_UTIL:-0.90}' TP='${TP:-8}' NO_MTP='${NO_MTP:-}' MTP_TOKENS='${MTP_TOKENS:-}' TOOL_PARSER='${TOOL_PARSER:-}' ATTN_BACKEND='${ATTN_BACKEND:-}' nohup bash '$S/vllm_glm52_nvfp4.sh' > '$LOG' 2>&1 < /dev/null & echo launched pid \$!"
