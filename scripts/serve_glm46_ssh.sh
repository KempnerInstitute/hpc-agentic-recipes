...
ssh -o BatchMode=yes "$NODE" "cd '$REPO_DIR'; mkdir -p '$LOG_DIR'; MODEL='$MODEL' API_PORT='$API_PORT' MAX_MODEL_LEN='${MAX_MODEL_LEN:-131072}' GPU_UTIL='${GPU_UTIL:-0.90}' ENFORCE_EAGER='${ENFORCE_EAGER:-}' NO_MTP='${NO_MTP:-}' MTP_TOKENS='${MTP_TOKENS:-}' PERF='${PERF:-}' nohup bash '$S/vllm_glm46.sh' > '$LOG' 2>&1 < /dev/null & echo launched pid \$!"
