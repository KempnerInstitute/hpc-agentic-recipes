...
ssh -o BatchMode=yes "$NODE" "cd '$REPO_DIR'; MODEL='$MODEL' SERVED_NAME='$SERVED_NAME' API_PORT='$API_PORT' MAX_MODEL_LEN='${MAX_MODEL_LEN:-40960}' GPU_UTIL='${GPU_UTIL:-0.90}' TP='${TP:-8}' PP='${PP:-}' QUANT='${QUANT:-}' TOOL_PARSER='${TOOL_PARSER:-}' REASONING_PARSER='${REASONING_PARSER-qwen3}' EXTRA_ARGS='${EXTRA_ARGS:-}' nohup bash '$S/vllm_qwen3.sh' > '$LOG' 2>&1 < /dev/null & echo launched pid \$!"
