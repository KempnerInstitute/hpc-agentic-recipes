...
ssh -o BatchMode=yes "$NODE" "cd '$REPO_DIR'; ${GPU:+CUDA_VISIBLE_DEVICES=$GPU} MODEL='$MODEL' API_PORT='$API_PORT' MAX_MODEL_LEN='${MAX_MODEL_LEN:-32768}' GPU_UTIL='${GPU_UTIL:-0.90}' TP='${TP:-1}' SPEC_DRAFT='${SPEC_DRAFT:-}' SPEC_TOKENS='${SPEC_TOKENS:-}' KV_FP8='${KV_FP8:-}' EXTRA_ARGS='${EXTRA_ARGS:-}' nohup bash '$S/vllm_gemma4.sh' > '$REPO_DIR/logs/vllm-gemma4.log' 2>&1 < /dev/null & echo launched pid \$!"
