#!/usr/bin/env bash
# Bring up Qwen3.8-27B over SSH on a node you already hold. Secondary path: prefer serve.sbatch.
...
NODE="${1:-${NODE:-${QWEN38_27B_NODE:-}}}"
[ -n "$NODE" ] || { echo "usage: $(basename "$0") <node>   (or set NODE, or QWEN38_27B_NODE in common/site.conf)" >&2; exit 2; }
LOG="$LOG_DIR/vllm-qwen3.8-27b-$API_PORT.log"
echo "launching qwen3.8-27b on $NODE:${API_PORT}   (GPU ${GPU:-all})"
...
echo "endpoint: http://$NODE:$API_PORT/v1   (served model: qwen3.8-27b)"
