...
| [`h200-4-nodes2`](h200-4-nodes2/README.md) | 2 H200 nodes, 4 GPUs each, TP4 x PP2 | does not run | n/a | Blocked |

Use the RTX variant. The H200 one was run end to end and fails during engine initialization, inside the
CUTLASS w8a8 kernel dispatch, because Hopper has no FP4 tensor cores for the expert layers. That page
records the traceback so the result does not have to be rediscovered.
