...
With 13.8B parameters active per token it is also the fastest large model in this repo, at 106.5 tok/s on a
single stream. The checkpoint carries a 19.8B speculative head, the DSpark module, which this hardware cannot
run; that would normally be the way to raise single-stream rate further. See
[Known limits](rtx-8/README.md#known-limits).

## Hardware variants

| Variant | Shape | Single stream | Aggregate | Status |
| --- | --- | --- | --- | --- |
| [`rtx-8`](rtx-8/README.md) | 1 RTX PRO 6000 node, 8 GPUs, TP8 | 106.5 tok/s | 5745 at c=1024, rising | Validated |
...
so 5745 tok/s is a floor rather than a ceiling. Measured with protocol slope(128,1152) over output tokens
