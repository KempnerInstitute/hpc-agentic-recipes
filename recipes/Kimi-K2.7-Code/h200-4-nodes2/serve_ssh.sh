...
HEAD="${1:-${KIMI_HEAD:-}}"
WORKER="${2:-${KIMI_WORKER:-}}"
if [ -z "$HEAD" ] || [ -z "$WORKER" ]; then
  echo "usage: $(basename "$0") <head_node> <worker_node>   (or set KIMI_HEAD and KIMI_WORKER in common/site.conf)" >&2
  exit 2
fi
