#!/usr/bin/env bash
# Serve gemma-4-31B-it on one H200 GPU: TP1, FP8 weights, prefix caching on.
# FP8 is the default because this checkpoint is dense and memory bandwidth bound, so halving the bytes
# read per token buys most of a proportional speedup. FP8 measured 52 percent faster than bf16 on this GPU. bf16 also fits the full 256K context here, so FP8 is a speed choice rather than a memory one.
