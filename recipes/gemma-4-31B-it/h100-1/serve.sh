#!/usr/bin/env bash
# Serve gemma-4-31B-it on one H100 GPU: TP1, FP8 weights, prefix caching on.
# FP8 is the default because this checkpoint is dense and memory bandwidth bound, so halving the bytes
# read per token buys most of a proportional speedup. FP8 measured 69 percent faster than bf16 on this GPU, and on an 80 GB card it is also what leaves room for context past the 32K default.
